...
function encryptData(plaintext, secret, algo) {
  ...
  return encrypt(secret, JSON.stringify(obj), algo);
}

function decryptData(ciphertext, secret) {
  const pt = decrypt(secret, ciphertext);
  const obj = JSON.parse(pt);
  const hmac = digest(secret, JSON.stringify(obj.pt));
...
module.exports = function(connect) {
  ...
  function MySQLStore(options) {
    ...
    if (this.enableCleanup) {
      const sql = 'DELETE FROM `' + this.table + '` WHERE `sid` IN (' +
        'SELECT temp.sid FROM (' +
          'SELECT `sid` FROM `' + this.table + '` WHERE `expires` > 0 AND `expires` < UNIX_TIMESTAMP()' +
        ') AS temp' +
      ');'
