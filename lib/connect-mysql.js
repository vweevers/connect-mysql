'use strict'

/*!
 * connect-mysql
 * Author: Nathan LaFreniere <nlf@andyet.net>
 */

var util = require('util');
var crypto = require('crypto');
var explain = require('explain-error');
var mysql = require('mysql');
var noop = function() {};

function isFunction(obj) {
  return Object.prototype.toString.call(obj) == '[object Function]';
}

function isNumber(obj) {
  return Object.prototype.toString.call(obj) == '[object Number]';
}

function encryptData(plaintext, secret, algo) {
  var hmac = digest(secret, plaintext);

  var obj = {
    hmac: hmac,
    pt: plaintext
  };

  var ct = encrypt(secret, JSON.stringify(obj), algo);

  return ct;
}

function decryptData(ciphertext, secret) {
  var pt = decrypt(secret, ciphertext);
  var obj = JSON.parse(pt);
  var hmac = digest(secret, JSON.stringify(obj.pt));

  if (hmac != obj.hmac) {
    throw 'Encrypted session was tampered with!';
  }

  return obj.pt;
}

function digest(key, obj) {
  var hmac = crypto.createHmac('sha512', key);
  hmac.setEncoding('hex');
  hmac.write(obj);
  hmac.end();
  return hmac.read();
}

function encrypt(key, pt, algo) {
  algo = algo || 'aes-256-ctr';
  pt = (Buffer.isBuffer(pt)) ? pt : new Buffer(pt);

  var cipher = crypto.createCipher(algo, key);
  var ct = [];

  ct.push(cipher.update(pt, 'buffer', 'hex'));
  ct.push(cipher.final('hex'));

  return ct.join('');
}

function decrypt(key, ct, algo) {
  algo = algo || 'aes-256-ctr';
  var cipher = crypto.createDecipher(algo, key);
  var pt = [];

  pt.push(cipher.update(ct, 'hex', 'utf8'));
  pt.push(cipher.final('utf8'));

  return pt.join('');
}


module.exports = function(connect) {
  var Store = connect.Store || connect.session.Store;

  function MySQLStore(options) {
    Store.call(this, options);

    this.table = options.table || 'sessions';
    this.numRetries = options.retries === 0 ? 0 : options.retries || 3;
    this.enableCleanup = options.cleanup == null ? true : !!options.cleanup;
    this.secret = options.secret;
    this.algorithm = options.algorithm || 'aes-256-ctr';
    this.config = options.config;
    this.pool = mysql.createPool(this.config);

    var cleanupQuery = 'DELETE FROM `' + this.table + '` WHERE id IN (' +
      'SELECT temp.id FROM (' +
        'SELECT `id` FROM `' + this.table + '` WHERE `expires` > 0 AND `expires` < UNIX_TIMESTAMP()' +
      ') AS temp' +
    ');'

    var nodeCleanup = function() {
      this.query(function(connection, done) {
        connection.query(cleanupQuery, function(err) {
          done(err);
        });
      }, noop);
    }.bind(this);

    if (this.enableCleanup) {
      // TODO: run once on startup
      setInterval(nodeCleanup, 900000).unref();
    }
  }

  util.inherits(MySQLStore, Store);

  MySQLStore.prototype.query = function(query, callback) {
    // TODO: always require a callback
    callback = callback || noop;

    var pool = this.pool,
      config = this.config,
      tries = 0,
      maxTries = this.numRetries + 1,
      error = function(err) {
        if (err.code === 'PROTOCOL_CONNECTION_LOST') {
          retry();
        } else callback(err);
      },
      release = function(connection) {
        return function(err, value) {
          connection.removeListener('error', error);
          connection.release();

          if (err) callback(err);
          else callback(null, value);
        };
      },
      execute = function(connection) {
        connection.on('error', error);
        try {
          query(connection, release(connection));
        } catch (err) {
          retry();
        }
      },
      retry = function(prevErr) {
        if (tries < maxTries) {
          tries++;
          try {
            pool.getConnection(function(err, connection) {
              if (err) callback(err);
              else execute(connection);
            });
          } catch (err) {
            retry(err);
          }
        } else if (prevErr) {
          callback(explain(prevErr, "Connection failed too many times in a row"));
        } else {
          callback(new Error("Connection failed too many times in a row"));
        }
      };

    retry();
  };

  MySQLStore.prototype.get = function(sid, callback) {
    var self = this;

    this.query(function(connection, done) {
      connection.query('SELECT `session` FROM `' + self.table +
        '` WHERE `sid` = ?', [sid],
        function(err, result) {
          if (result && result[0] && result[0].session) {
            try {
              var session = JSON.parse(result[0].session);

              if (self.secret) {
                session = JSON.parse(decryptData(session, self.secret, self.algorithm));
              }

              done(null, session);
            } catch (cryptoErr) {
              done(cryptoErr);
            }
          } else {
            done(err);
          }
        });
    }, callback);
  };

  MySQLStore.prototype.set = function(sid, session, callback) {
    var expires = new Date(session.cookie.expires).getTime() / 1000;
    var self = this;

    session = JSON.stringify(session);

    if (this.secret) {
      session = encryptData(session, this.secret, this.algorithm);
    }

    this.query(function(connection, done) {
      connection.query('INSERT INTO `' + self.table +
        '` (`sid`, `session`, `expires`) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE `session` = ?, `expires` = ?', [
          sid, session, expires, session, expires
        ],
        function(err) {
          done(err);
        });
    }, callback);
  };

  MySQLStore.prototype.destroy = function(sid, callback) {
    var self = this;

    this.query(function(connection, done) {
      connection.query('DELETE FROM `' + self.table +
        '` WHERE `sid` = ?', [sid],
        function(err) {
          done(err);
        });
    }, callback);
  };


  return MySQLStore;
};
